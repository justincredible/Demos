...
impl FxaaSystem {
    ...
    pub fn picking_texture(
        &self,
    ) -> std::cell::Ref<'_, Option<glium::texture::UnsignedTexture2d>> {
...
where
    T: Surface,
    F: FnMut(&mut MultiOutputFrameBuffer<'_>) -> R,
{
    ...
    if target_color.is_none() {
        let texture =
            glium::Texture2d::empty(&system.context, target_dimensions.0, target_dimensions.1)
                .unwrap();
    ...
    let output = draw(
        &mut MultiOutputFrameBuffer::with_depth_buffer(
            &system.context,
            [
                ("f_colour", target_color.to_color_attachment()),
                ("f_id", target_pickr.to_color_attachment()),
            ],
            target_depth,
        )
        .unwrap(),
